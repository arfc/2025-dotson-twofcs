...
\textcolor{black}{Considering sub-optimal solutions is useful for addressing
...
\gls{moo} for \textcolor{black}{their inability to sample a model's inferior
...
tools, \gls{temoa} \cite{decarolis_temoa_2010} and \textcolor{black}{the}
...
\textcolor{black}{\gls{ens}} \cite{wigeland_nuclear_2014-2} and an objective
...
% \textcolor{black}{I don't love the names of these two subsections...}
...
\begin{align}
    ...
    \textbf{CAP}_g &= \text{ the \textcolor{black}{electric} capacity of the \textit{g$^{th}$} 
    ...
    C^{fom}_{g} &= \text{the fixed \textcolor{black}{O\&M} costs of the \textit{g$^{th}$} 
    technology} \quad \left[\frac{\$}{MW}\right]\nonumber,\\
    C^{[fuel, vom]}_{g,t} &= \text{the variable \textcolor{black}{O\&M} costs of the 
...
\textcolor{black}{with} \gls{temoa}'s \gls{mga} algorithm and then passed the
...
\textcolor{black}{\gls{ens} \cite{wigeland_nuclear_2014,wigeland_nuclear_2014-1}}.
We assumed that the nuclear fuel cycles represented in the \gls{set} are
mutually exclusive. \textcolor{black}{That is, decision-makers will choose a
...
\textcolor{black}{Equation \ref{eqn:min-tradeoff} represents the least improvement per unit
...
the \textcolor{black}{\gls{ens}} \cite{wigeland_nuclear_2014-2}.
